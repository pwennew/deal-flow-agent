...
_SYSTEM_PROMPT = """You are qualifying PE carve-out deals for Larkhill & Company, a separation execution firm. Larkhill runs the Separation Management Office (SMO) for PE-backed carve-outs.

For each deal, score against these criteria (0-100 each):

1. PE_BUYER: Is a PE firm on the BUY side? (Larkhill sells to PE deal teams)
   - 100 = Named PE firm acquiring a division/unit from a corporate parent
   - 50 = PE involvement likely but not confirmed
   - 0 = No PE buyer. IMPORTANT: If the SELLER is a PE firm (e.g. "New Mountain Capital sells Qualus"), the PE firm is EXITING, not buying — score 0. Larkhill needs a PE BUYER, not a PE seller. Also score 0 for corporate-to-corporate deals with no PE involvement on the buy side.
...
Respond with a JSON array of objects, one per deal, in the same order as the input:
{
  ...
  "timing_score": 0-100,
  "pe_firm": "ALL PE firms on the buy side, comma-separated (e.g. 'Apollo, Bain Capital'). If multiple firms are bidding or co-investing, list every one. If the article says 'private equity' or 'PE firms' without naming them, write 'Unnamed PE' so we know PE involvement exists. Empty string ONLY if there is genuinely no PE buyer involvement at all.",
  "reasoning": "brief explanation",
  "recommended_action": "pursue" or "monitor" or "discard"
}

"pursue" = larkhill_fit >= 65 — high-quality carve-out opportunity for Larkhill
...
def _qualify_batch(alerts: list[DealAlert]) -> list[QualifiedAlert]:
    ...
    numbered = "\n".join(
        ...
        + f"\n   Target: {a.target_company}, Seller: {a.seller}"
    ...
    user_msg = f"Qualify these carve-out deals for Larkhill:\n\n{numbered}"
        ...
        try:
            ...
            for i, alert in enumerate(alerts):
                if i < len(results):
                    ...
                    # Hard cap: if PE_BUYER = 0 (no PE buyer, or PE is
                    # the seller), this is not a Larkhill opportunity
