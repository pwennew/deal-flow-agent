#!/usr/bin/env python3
"""
Merge early-stage signals with completed/definitive signals and reprocess
"""
...
def merge_and_process():
    """Merge all signals and reprocess through filters."""

    # Load both signal sets
    ...
    with open('web_search_early_stage_signals.json', 'r') as f:
        early_stage_signals = json.load(f)

    # Merge
    all_signals = completed_signals + early_stage_signals

    print(f"Loaded {len(completed_signals)} completed/definitive signals")
    print(f"Loaded {len(early_stage_signals)} early-stage signals")
    ...
    # Write to CSV
    print("\n=== Writing to CSV ===")
    output_file = 'web_search_deal_signals_2025_complete.csv'
